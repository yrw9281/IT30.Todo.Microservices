...
在這個教學系列中，一度想要把 GraphQL 的介紹拿掉，因為 GraphQL 的靈活與高彈性，很容易造成大量的 Queries 在同一秒內產生，理論上我從先前做好的 Repository 這裡來當作實作的一部分非常不明智，不只因為 Aggregate 的 ValueObject 會造成 Filtering 的困擾，還有濫用 GraphQL 會有高併發的情況出現，現有的 Infrastructure Layer 根本沒辦法很好的應付這種情況。GraphQL 的水深遠沒有我所介紹的這麼膚淺，效能優化和合理的架構設計對 GraphQL 來說是至關重要的，這裡只有介紹淺薄的應用，帶領讀者入門，之後的修行還是得靠個人。
